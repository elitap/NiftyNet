...
def filter_using_coarse_out(res_itk, filter_itk, dilation_radius):
    res_np = sitk.GetArrayFromImage(res_itk).astype(np.uint8)
    filter_np = sitk.GetArrayFromImage(filter_itk).astype(np.uint8)
    for key, value in enumerate(LABELS):
        filter_cp = filter_np.copy()
...
def postproc(resdir, postprocdir, filter_dir, foreground_filter, dilation_radius):
    ...
    for res_file in os.listdir(resdir):
        ...
        if os.path.isfile(full_file):
            ...
            for filter in os.listdir(filter_dir):
                ...
                if os.path.isfile(full_filter) and id in filter and foreground_filter in filter:
                    ...
                    new_res_itk = old_postproc(dilation_radius, filter_itk, postprocdir, res_file, res_itk)
...
def old_postproc(dilation_radius, filter_itk, res_itk):
