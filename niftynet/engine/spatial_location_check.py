...
class SpatialLocationCheckLayer(Layer):
    ...
    def __init__(self,
        ...
        self.min_ratio = max(float(minimum_ratio), 0.0)
        self.min_nlabels = int(min_numb_labels)
    ...
    def __volume_level_eval(self):
        ...
        if len(uniq) < self.min_nlabels:
            return False
        #this requires the batch to contain some background, when called with default settings
        for x in self.compulsory_labels:
            if x not in uniq:
                return False
        return True

    def set_compulsory(self, compulsory):
    ...
    def layer_op(self, location, spatial_rank):
        ...
        satisfied = np.logical_and(np.asarray(is_enough, dtype=bool), np.asarray(is_compulsory, dtype=bool))
        if np.sum(satisfied) >= self.min_nlabels:
            print("ratio is ",ratio_[satisfied])
