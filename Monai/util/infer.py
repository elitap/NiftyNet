...
def sliding_window_inference(
    ...
    for curr_slice in slices:
        if len(curr_slice) == 3:
            if mask is not None:
                if mask[0, :, curr_slice[0], curr_slice[1], curr_slice[2]].sum() > 0.0:
                    input_slices.append(inputs[0, :, curr_slice[0], curr_slice[1], curr_slice[2]])
                    slice_loc.append(curr_slice)
            else:
                input_slices.append(inputs[0, :, curr_slice[0], curr_slice[1], curr_slice[2]])
                slice_loc.append(curr_slice)
        else:
        ...
        if len(input_slices) == sw_batch_size:
            slice_batches.append(torch.stack(input_slices))
            input_slices.clear()

    # append remaining slices not adding up to a batch
    slice_batches.append(torch.stack(input_slices))
    ...
    for window_id, slice_index in enumerate(range(0, len(slice_loc), sw_batch_size)):
        slice_index_range = range(slice_index, min(slice_index + sw_batch_size, len(slice_loc)))

        # store the result in the proper location of the full output. Apply weights from importance map.
        for curr_index in slice_index_range:
            curr_slice = slice_loc[curr_index]
            if len(curr_slice) == 3:
